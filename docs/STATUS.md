...
**Last updated**: 2026-05-21（TestFlight 0.1.0 (7) 上传）
**Last updater**: Codex
...
- 2026-05-21 TestFlight 第四版已上传：发布前 `npm run typecheck`、`npm run lint`、`npm test`（26 files / 87 tests）和 `git diff --check` 通过。EAS build `e1acde16-47dd-40a4-944f-4a9ccad94fd2` 已成功生成 `0.1.0 (7)` IPA，并通过 EAS Submit `a04c88d6-3eda-477a-9e41-2c3ca212629f` 上传到 App Store Connect；Apple 正在处理，TestFlight 页面为 `https://appstoreconnect.apple.com/apps/6770225506/testflight/ios`。
...
- 2026-05-21 TestFlight 第三版问题修复：真机沙盒检查发现 99 条 capture 全部持久化为旧 iOS app container 绝对路径，其中 93 条因 TestFlight 更新后 container UUID 变化被误标 `conflicted`（87 条 `local_capture_incomplete`、6 条 `recording_manifest_unreadable`）；导出的沙盒 `captures/<id>/manifest.json` 和 `.complete` 对 93 条均存在。新增 schema v5 migration 把 `captures.local_path` 规范化为 `captures/<id>`，运行时统一解析到当前 `documentDirectory`，reconcile 会自动修复旧绝对路径，并按 `acked_at` / `uploaded_at` 恢复已误标的本地冲突记录。X1 U 盘导入页新增基于 `recording_annotations.kind='x1_import'` 的持久导入历史，返回页面后仍显示已导入项并可打开录音；U 盘导入会读取音频真实时长写入 recording/manifest，详情播放器也会用 `expo-av` 实际加载时长修正 0 秒录音的总时长与 +15s seek。验证：`npm run typecheck`、`npm run lint`、`npm test`（26 files / 87 tests）通过。
