...
让 Mac 端 Orbit 监听 Orbit Mobile 写入的 iCloud Drive `inbox/<id>/`，校验 manifest/hash 后创建 Thought，并通过 `processed/<id>/.acked` 或 `failed/<id>/.failed.json` 回写结果。
...
- [x] 成功 ingest 创建 Thought 并移动到 `processed/<id>/.acked`
